...
bookingForm && bookingForm.addEventListener('submit', e => {
  ...
  // ── EmailJS keys (unchanged) ──────────────────────────────────────────
  const EMAILJS_PUBLIC_KEY  = 'bPMLRCg_bNDFfBehx';
  const EMAILJS_SERVICE_ID  = 'service_9v9gdw3';
  const EMAILJS_TEMPLATE_ID = 'template_7grqwii';

  const roomLabels = {
    deluxe:       'Deluxe Garden Room',
    ocean:        'Ocean Grand Suite',
    presidential: 'Presidential Penthouse'
  };
  const roomVal = document.getElementById('roomType')?.value || 'deluxe';

  const templateParams = {
    guest_name:   `${document.getElementById('firstName').value.trim()} ${document.getElementById('lastName').value.trim()}`,
    ...
    room_type:    roomLabels[roomVal] || roomVal,
    requests:     document.getElementById('requests').value.trim() || '—',
    submitted_at: new Date().toLocaleString('fr-FR', { dateStyle: 'full', timeStyle: 'short' })
  };

  // ── Netlify Forms AJAX submission ─────────────────────────────────────
  // Because e.preventDefault() stops the native POST, we replicate it with
  // fetch() so the submission still appears in the Netlify dashboard.
  // URLSearchParams serialises every named field, including the hidden
  // form-name input, exactly as a native POST would.
  const netlifyPayload = new URLSearchParams(new FormData(bookingForm)).toString();
  ...
  const emailjsSubmit = emailjs.send(EMAILJS_SERVICE_ID, EMAILJS_TEMPLATE_ID, templateParams);
  // ── End EmailJS block ─────────────────────────────────────────────────

  // Wait for BOTH to settle, then decide what to show
  Promise.allSettled([netlifySubmit, emailjsSubmit]).then(([netlifyResult, emailResult]) => {

    // Log any failures to the console for debugging — don't alert the user
    if (netlifyResult.status === 'rejected') {
      console.error('Netlify submission error:', netlifyResult.reason);
    }
    if (emailResult.status === 'rejected') {
      console.error('EmailJS error:', emailResult.reason);
    }

    // ✅ As long as at least one channel succeeded, show the success state.
    // If BOTH failed (e.g. no internet), restore the button for a retry.
    const bothFailed =
    ...
    if (bothFailed) {
      btn.disabled = false;
      btn.querySelector('.btn-text').textContent = 'Confirmer la réservation';

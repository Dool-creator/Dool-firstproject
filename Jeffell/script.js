...
bookingForm && bookingForm.addEventListener('submit', e => {
  ...
  // STEP 2 — Create an Email Service (Gmail, Outlook, etc.) and copy its ID below
  // STEP 3 — Create an Email Template and copy its ID below
  //          In the template body use these variables (copy/paste as-is):
  //            {{guest_name}}  {{guest_email}}  {{guest_phone}}
  //            {{check_in}}    {{check_out}}    {{guests}}
  //            {{room_type}}   {{requests}}     {{submitted_at}}
  // STEP 4 — Go to Account → API Keys and copy your Public Key below
  // STEP 5 — In your EmailJS template, set "To Email" to your address:
  //          ▶▶  YOUR_EMAIL@example.com  ◀◀  (replace inside EmailJS dashboard)

  const EMAILJS_PUBLIC_KEY  = 'bPMLRCg_bNDFfBehx';   // ← paste your EmailJS Public Key here
  const EMAILJS_SERVICE_ID  = 'service_9v9gdw3';   // ← paste your EmailJS Service ID here
  const EMAILJS_TEMPLATE_ID = 'template_7grqwii';  // ← paste your EmailJS Template ID here
  ...
  emailjs.send(EMAILJS_SERVICE_ID, EMAILJS_TEMPLATE_ID, templateParams)
    .then(() => {
      // ✅ Email sent — show success state (same as before)
      bookingForm.style.display = 'none';
      bookingSuccess.classList.add('show');
    })
    .catch(err => {
      // ❌ Send failed — restore button so user can retry
